...
const main = async () => {
    ...
    try {
        const info = await client.query("SELECT current_database() AS database, current_schema() AS schema, version()");
        imprimirFila("Conexion", info.rows[0]);
        const tablasRes = await client.query(`SELECT table_name
